        ...
        string materialsContainingThisName = null)
    {
        ...
        MapSharedMaterialsRecursively(
            ...
            (mat) =>
            {
                ...
                if (replaceThemWith.HasProperty(ShaderUtils._GMatCap))
                    _GMatCap = replaceThemWith.GetTexture(ShaderUtils._GMatCap);

                if (replaceThemWith.HasProperty(ShaderUtils._FMatCap))
                    _FMatCap = replaceThemWith.GetTexture(ShaderUtils._FMatCap);
                ...
                if (_GMatCap != null)
                    copy.SetTexture(ShaderUtils._GMatCap, _GMatCap);

                if (_FMatCap != null)
                    copy.SetTexture(ShaderUtils._FMatCap, _FMatCap);
