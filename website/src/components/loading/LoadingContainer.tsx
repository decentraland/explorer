...
export const LoadingContainer: React.FC<LoadingContainerProps> = (props) => {
  ...
  const subMessage = state.pendingScenes > 0 ? state.message || 'Loading scenes...' : state.status
  const percentage = Math.min(
    state.initialLoad ? (state.loadPercentage + state.subsystemsLoad) / 2 : state.loadPercentage,
    100
  )
  const show = state.showLoadingScreen || state.waitingTutorial
  const withImages = state.initialLoad || props.loginStage !== 'completed' || state.waitingTutorial
  return (
    <React.Fragment>
      {show && (
        <LoadingMessage
