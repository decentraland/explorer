...
export class TeleportController {
    ...
    public static goTo(x:number, y:number, teleportMessage?:string) : {message:string, success:boolean} {
        if (!teleportMessage) {
            teleportMessage = `Teleporting to ${x}, ${y}...`
        }
        const insideCoords =  x > parcelLimits.minLandCoordinateX &&
        ...
        if ( insideCoords ) {
            teleportObservable.notifyObservers({
                x: x,
                y: y,
                text: teleportMessage} as any)
            
            TeleportController.ensureTeleportAnimation()
            
            return { message:teleportMessage, success:true }
        } else {
            teleportMessage = `Coordinates are outside of the boundaries. Limits are from ${parcelLimits.minLandCoordinateX} to ${parcelLimits.maxLandCoordinateX} for X and ${parcelLimits.minLandCoordinateY} to ${parcelLimits.maxLandCoordinateY} for Y`
            return { message:teleportMessage, success:false }
        }
    }
}