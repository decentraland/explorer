...
export const ErrorNetworkMismatch: React.FC<ErrorNetworkMismatchProps> = (props: ErrorNetworkMismatchProps) => {
  const { tld = 'zone', web3Net = 'mainet', tldNet = 'ropsten' } = props.details || {}
  return (
      ...
      <div className="errormessage">
        <div className="errortext col">
          ...
          <div className="givesomedetailof">
            We detected that you are entering the <strong id="tld">{tld || 'zone'}</strong> domain with your Ethereum
            wallet set to <strong id="web3Net">{web3Net || 'mainnet'}</strong>.
          </div>
          <div className="givesomedetailof">
            To continue, please change the Ethereum network in your wallet to{' '}
            <strong id="web3NetGoal">{tldNet || 'ropsten'}</strong> and click "Reload".
          </div>
          <div className="cta">
            <button
              className="retry"
