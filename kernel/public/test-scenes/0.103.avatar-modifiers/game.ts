...
hideAvatarsEntity.addComponent(
  new AvatarModifierArea({ area: { box: new Vector3(SIZE, SIZE, SIZE) }, modifiers: [AvatarModifiers.HIDE_AVATARS] })
)
hideAvatarsEntity.addComponent(
  new Transform({ position: new Vector3(5, SIZE / 2, 8), rotation: new Quaternion(0, 0.5) })
)
...
box1.addComponent(
  new Transform({
    position: new Vector3(5, 0.125, 8),
    scale: new Vector3(SIZE, 0.25, SIZE),
    rotation: new Quaternion(0, 0.5)
  })
)
...
disablePassportsEntity.addComponent(
  new AvatarModifierArea({
    area: { box: new Vector3(SIZE, SIZE, SIZE) },
